function [stepdis_seg, knndis_seg, knnportion_seg,spdcong_seg,dircong_seg] = projected_xx_measures_segs(x_measure,x_knnbase,d,tgrid,speed_tc,plot_result,k)
...
for m = 1:n_manifold
    ...
    for i=1:n_seg
        ...
        knnportion_seg(m,i) = sum(tmptbl(:,2)>0)/(d(i+1)-d(i)); % knn distribution on tc
%         knnportion_seg(m,i) = sum(tmptbl(:,2)>0)/size(x_tc,1); % knn distribution on tc
    end
    knn_in_tc = [knn_in_tc, {tmptbl(:,2)>0}];
    x_knnbase_all = [x_knnbase_all;x_tc];
end

% seg_congruence _new
dircong_seg = get_dir_congruence(tgrid,x_measure,x_knnbase_all,speed_tc,k);
